...
function DashboardPage() {
    ...
    useEffect(() => {
        const fetchTransactions = async () => {
            if (token && user) { // Adicionada verificação de 'user' para garantir que os dados já chegaram
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        setTransactionsError(errorData.message || 'Falha ao buscar histórico.');
                    }
                } catch (error) {
    ...
    }, [token, authIsLoading, user]); // Adicionado 'user' à dependência

    if (authIsLoading) {
        return <div><p>Carregando informações do usuário...</p></div>;
    }
    ...
    return (
        <div className="dashboard-container" style={{ fontFamily: 'Arial, sans-serif', padding: '20px', maxWidth: '900px', margin: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #30c0f0', paddingBottom: '10px' }}>
                {}
                <h2>Painel de Controle <span style={{ fontSize: '0.8em', color: '#777' }}>(Nave: {user.name || 'Piloto'})</span></h2>
                <button onClick={logoutAction} className="btn-logout" style={{padding: '8px 15px', backgroundColor: '#c03030', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.9em'}}>
                    Desconectar (Logout)
                </button>
            </div>
            ...
            <div className="welcome-message" style={{ margin: '20px 0', padding: '15px', backgroundColor: '#f0f8ff', borderRadius: '5px', borderLeft: '5px solid #007bff' }}>
                {}
                <h3>Bem-vindo(a) de volta, Comandante {user.name}!</h3>
            ...
            <div className="account-summary" style={{ display: 'flex', gap: '20px', margin: '20px 0' }}>
                <div className="account-balance" style={{ flex: 1, padding: '15px', backgroundColor: '#e6f7ff', borderRadius: '5px', textAlign: 'center' }}>
                    ...
                    <p className="balance-amount" style={{ fontSize: '2em', fontWeight: 'bold', color: '#007bff', margin: '10px 0' }}>
                        {}
                        CR$ {user.balance?.toFixed(2) || '0.00'}
                    </p>
                </div>
                <div className="dashboard-actions" style={{ flex: 1, padding: '15px', backgroundColor: '#e9ecef', borderRadius: '5px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '10px' }}>
                    <h4>Ações Rápidas:</h4>
                    <Link to="/transferencia" style={{ display: 'block', width: '80%', textAlign: 'center', padding: '10px 15px', backgroundColor: '#007bff', color: 'white', textDecoration: 'none', borderRadius: '5px', marginBottom: '10px' }}>
                        Nova Transferência
                    </Link>
                    <Link to="/deposito" style={{ display: 'block', width: '80%', textAlign: 'center', padding: '10px 15px', backgroundColor: '#28a745', color: 'white', textDecoration: 'none', borderRadius: '5px', marginBottom: '10px' }}>
                        Realizar Depósito
                    </Link>
                    <Link to="/saque" style={{ display: 'block', width: '80%', textAlign: 'center', padding: '10px 15px', backgroundColor: '#dc3545', color: 'white', textDecoration: 'none', borderRadius: '5px' }}>
                        Realizar Saque
                    </Link>
                </div>
            </div>
            ...
            <div className="transaction-history" style={{ marginTop: '30px', borderTop: '1px solid #ccc', paddingTop: '20px' }}>
                ...
                {transactionsLoading && <p>Carregando histórico...</p>}
                            ...
                            <li key={tx._id} style={{ border: '1px solid #eee', padding: '15px', marginBottom: '10px', borderRadius: '5px', backgroundColor: '#f9f9f9' }}>
                                ...
                                {tx.parteContraria && tx.parteContraria.email && (
                                    <p style={{margin: '5px 0', fontSize: '0.9em'}}>
                                        {}
                                        <strong>{tx.tipo.includes('enviada') ? 'Para:' : 'De:'}</strong> {tx.parteContraria.name || tx.parteContraria.email}
                                    </p>
                                )}
